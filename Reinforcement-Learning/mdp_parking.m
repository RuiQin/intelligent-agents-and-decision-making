function [T, R] = mdp_parking(N, occupiedProb, parkingRewards, A)
...
R = zeros(totalState + 1, 1);

%define the cost
collision = -100; driving = -0.5;
...
T = zeros(totalState + 1,totalState + 1, A);  % the last state is termianl state
...
%define the transition and reward matrix
...
for i = 1:totalState
    ...
    if isParked == 0
        ...
        % DRIVE = 2
        if curSpot == N * 2
            T(i,1,2) = 1 - occupiedProb(1);  %{L=20,O,P=F}->{L=1,O=F,P=F}
            T(i,3,2) = occupiedProb(1); %{L=20,O,P=F}->{L=1,O=T,P=F}
        else    % L = 1,2,...,19
            if isOccupied == 1
